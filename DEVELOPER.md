...
AI provider contract helpers and tool orchestration types.
...
- You need **typed AI helpers**, **prompt composition**, **model-facing contracts** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/ai` instead of reaching into app-specific internals.
...
| `framework/libraries/ai` | Nested publishable library package. |
| `framework/libraries/ai/src` | Public runtime source and exported modules. |
| `framework/libraries/ai/tests` | Unit and contract verification where present. |
...
| Field | Value |
| --- | --- |
| Direct Dependencies | `@platform/schema` |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `createToolContract`, `createModelRegistry`, `invokeModel`, `runGuardrails`, `createProviderError` |
...
- Hosts should import from `@platform/ai`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/ai"]
...
```ts
import { packageId, packageDisplayName, createToolContract } from "@platform/ai";

console.log(packageId, packageDisplayName);
console.log(typeof createToolContract);
```
