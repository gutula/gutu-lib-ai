...
| Primary focus | typed AI helpers, prompt composition, model-facing contracts |
...
- This repo currently publishes `@platform/ai` as the legacy compatibility package id while the migration to `@gutu/ai` is completed.
...
| Named Exports | 23 | `packageId`, `packageDisplayName`, `packageDescription`, `createToolContract`, `createModelRegistry`, `invokeModel`, `runGuardrails`, `createProviderError` |
...
console.log(packageId, packageDisplayName, typeof createToolContract);
```

Use the root repo scripts for day-to-day work after the workspace is bootstrapped, or run the nested package directly from `framework/libraries/ai` if you need lower-level control.
